    ...
    return Promise.resolve().then(() => {
      ...
      return _amqplib.default.connect(_url.default.format(amqpUrl), connectionOptions).then(connection => {
        ...
        connection.on('close', err => {
          ...
          (0, _helpers.wait)(this.reconnectTimeInSeconds * 1000).then(() => {
      ...
      this.logger.debug(`Emitted disconnect event`); // Connection failed...

      this._currentConnection = null;
      this.logger.debug('_currentConnection set to null'); // TODO: Probably want to try right away here, especially if there are multiple brokers to try...
